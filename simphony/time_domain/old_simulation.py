...
class TimeSim(Simulation):
    ...
    def tarjan_scc(self, graph, max_size):
        ...
        def choose_edge_to_remove(subgraph, max_size):
            ...
            for u in subgraph:
                # Iterate over a copy of the list with index, so we can restore later.
                for idx in range(len(subgraph[u])):
                    v = subgraph[u][idx]
                    # Temporarily remove edge (u, v)
                    backup = subgraph[u][idx]
                    del subgraph[u][idx]
        ...
        def split_large_scc(g, scc, max_size, max_iter=100):
            ...
            while iter_count < max_iter:
                new_sccs = compute_scc_no_split(subgraph)
                if all(len(comp) <= max_size for comp in new_sccs):
                    return new_sccs
                # Choose the best edge to remove.
                edge_to_remove = choose_edge_to_remove(subgraph, max_size)
                if edge_to_remove is None:
                    # No candidate found; break to avoid infinite loop.
                    break
                u, idx, v = edge_to_remove
                # Remove the chosen edge permanently.
                subgraph[u].pop(idx)
                iter_count += 1
        ...
        for scc in initial_sccs:
            if len(scc) > max_size:
                split_comps = split_large_scc(graph, scc, max_size)
                result.extend(split_comps)
            else:
                result.append(scc)
        return result
    ...
    def create_passive_sub_netlists(
        ...
        for scc_comp_list in sccs:
            ...
            sub_nl, reconnect_passive_component = self.build_sub_netlist(
                scc_comp_set,
                filtered_conns,
                filtered_ports,
                removed_connections,
