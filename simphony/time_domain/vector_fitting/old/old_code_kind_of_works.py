...
def compute_phi_matrices(omega, poles_real, poles_complex):
    ...
    for i in range(num_poles_real):
        phi_real[:, i] = 1.0 / (1j * omega[:, 0] - poles_real[i])
    phi_complex = np.zeros((kbar, 2 * num_complex_pairs), dtype=complex)
    for i in range(num_complex_pairs):
        phi_complex[:, 2 * i] = 1.0 / (1j * omega[:, 0] - poles_complex[i]) + 1.0 / (
            1j * omega[:, 0] - np.conj(poles_complex[i])
        )
        phi_complex[:, 2 * i + 1] = 1j / (1j * omega[:, 0] - poles_complex[i]) - 1j / (
            1j * omega[:, 0] - np.conj(poles_complex[i])
        )
...
def compute_time_response(times, R0, Rr, Rc, pr, pc):
    ...
    for t_index in range(len(times)):
        ...
        # complex poles
        for n in range(nc):
            h[t_index, :, :] += 2 * np.real(Rc[:, :, n]) * np.exp(
                np.real(pc[n]) * t
            ) * np.cos(np.imag(pc[n]) * t) - 2 * np.imag(Rc[:, :, n]) * np.exp(
                np.real(pc[n]) * t
            ) * np.sin(
                np.imag(pc[n] * t)
            )
...
def complex_valued_ABCD(model):
    ...
    # A = block_diag(*A_n)
    poles_s = np.concatenate(
        [model.poles_real, model.poles_complex, np.conj(model.poles_complex)]
    )
    ...
    for n in range(len(model.poles_complex)):
        ...
        A_n.append(
            np.block(
                [
                    [
                        np.real(model.poles_complex[n]) * np.eye(rank),
                        np.imag(model.poles_complex[n]) * np.eye(rank),
                    ],
                    [
                        -np.imag(model.poles_complex[n]) * np.eye(rank),
                        np.real(model.poles_complex[n]) * np.eye(rank),
...
def FastVF(omega, H, order, options):
    ...
    if num_complex_pairs == 1:
        ...
        poles_complex = (
            (-alpha + 1j)
            * max(omega)
            * np.arange(1, num_complex_pairs + 1, dtype=complex)
            / num_complex_pairs
        )
        # poles_complex = poles_complex.reshape((len(poles_complex), 1))
    else:
        poles_complex = (-alpha + 1j) * (
            min(omega)
            + (max(omega) - min(omega))
            / (num_complex_pairs - 1)
            * np.arange(0, num_complex_pairs, dtype=complex)
        )
        # poles_complex = poles_complex.reshape((len(poles_complex), 1))
    ...
    while iter <= options.max_iterations:
        ...
        for q in range(qbar):
            for m in range(mbar):
                ...
                D_Hqm = np.diag(V_H)
                M[:kbar, nbar + 1 : nbar + num_real_poles + 1] = -np.real(
                    D_Hqm @ phi_real
                )
                M[:kbar, nbar + num_real_poles + 1 :] = -np.real(D_Hqm @ phi_complex)
                M[kbar:, nbar + 1 : nbar + num_real_poles + 1] = -np.imag(
                    D_Hqm @ phi_real
                )
                ...
                b[irow : irow + nbar, 0] = Q[:kbar, nbar + 1 :].T @ np.real(V_H) + Q[
                    kbar:, nbar + 1 :
                ].T @ np.imag(V_H)
        ...
        w = (
            1 + phi_real @ cw[:num_real_poles] + phi_complex @ cw[num_real_poles:]
        ...
        for i in range(num_complex_pairs):
            A[
                num_real_poles + 2 * (i + 1) - 2 : num_real_poles + 2 * (i + 1),
                num_real_poles + 2 * (i + 1) - 2 : num_real_poles + 2 * (i + 1),
            ] = np.array(
                [
            ...
            bw[
                num_real_poles + 2 * (i + 1) - 2 : num_real_poles + 2 * (i + 1), 0
            ] = np.array([2, 0])
        ...
        ind_rp = np.nonzero(np.abs(np.imag(new_poles)) < 10 * eps * np.abs(new_poles))[
            0
        ]  # I increased the constant 10 to 100 for now
        ...
        if options.enforce_stability:
            ...
            poles_complex = -np.abs(np.real(poles_complex)) + 1j * np.imag(
                poles_complex
            )
        ...
        if w_minus_one <= options.poles_estimation_threshold or options.debug:
            ...
            phi_real, phi_complex = compute_phi_matrices(
                omega, poles_real, poles_complex
            )
            ...
            for q in range(qbar):
                for m in range(mbar):
                    ...
                    real_indices = slice(
                        num_real_poles + 1, None, 2
                    )  # Equivalent to nr+2:2:end in MATLAB
                    imag_indices = slice(
                        num_real_poles + 2, None, 2
                    )  # Equivalent to nr+3:2:end in MATLAB
    ...
    print(
        "Warning: could not reach the desired modeling error within the allowed number of iterations\n"
    )
    return model
...
def main1():
    ...
    residues = np.reshape(residues[1:], (10, 1, 1))
    feedthrough = np.zeros((1, 1), dtype=complex)
    N = len(poles)

    f = np.linspace(0.1, 10 / (2 * np.pi), 100)

    def pole_residue_response(frequency, poles, residues, feedthrough):
        s = 2j * np.pi * (frequency)
        frequency_response = feedthrough[None, :, :] + np.sum(
            residues[None, :, :, :]
            / (s[:, None, None, None] - poles[None, :, None, None]),
            axis=1,
        )
    ...
    plt.plot(response[:, 0, 0])
    plt.show()

    model = FastVF(2 * np.pi * f, response, 20, None)
    y = ComputeModelResponse(
        2 * np.pi * f,
        model.R0,
        model.Rr,
        model.Rc,
...
def main2():
    ...
    def mzi(wl=1.55):
        return _mzi(wl=wl, long_wg={"length": 10.0}, short_wg={"length": 1000.0})

    f_min = speed_of_light / 1.555e-6
    f_max = speed_of_light / 1.545e-6
    f_center = 0.5 * (f_min + f_max)
    frequency = np.linspace(f_min, f_max, 1000)
    s_params = np.reshape(
        dict_to_matrix(mzi(wl=1e6 * speed_of_light / frequency))[:, 0, 1], (1000, 1, 1)
    )
    phase = np.unwrap(np.angle(s_params))
    s_params = s_params
    # s_params = np.exp(-1j*phase)*s_params

    model = FastVF((2 * np.pi * frequency) / 1e13, s_params, 20, None)

    y = ComputeModelResponse(
        (2 * np.pi * frequency) / 1e13,
        model.R0,
        model.Rr,
        model.Rc,
        model.poles_real,
    ...
    plt.show()
    plt.plot(frequency, np.abs(y[0, 0, :]))
    plt.plot(frequency, np.angle(y[0, 0, :]))
    plt.plot(frequency, np.abs(s_params[:, 0, 0]))
    plt.plot(frequency, np.angle(s_params[:, 0, 0]))
    plt.show()
