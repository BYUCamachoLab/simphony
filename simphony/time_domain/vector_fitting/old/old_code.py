...
def main1():
    import numpy as np
    poles = np.array([-1.3578, -1.2679, -1.4851 + 0.2443*1j,-1.4851 - 0.2443*1j, -0.8487 + 2.9019*1j,-0.8487 - 2.9019*1j, -0.8587+3.1752*1j,-0.8587-3.1752*1j, -0.2497+6.5369*1j, -0.2497-6.5369*1j])
    residues = np.array([0.1059, -0.2808, 0.1166, 0.9569 - 0.7639*1j,0.9569 + 0.7639*1j, 0.9357 - 0.7593 * 1j,0.9357 + 0.7593 * 1j, 0.4579-0.7406*1j,0.4579+0.7406*1j, 0.2405-0.7437*1j, 0.2405+0.7437*1j])
    ...
    y = ComputeModelResponse(2*np.pi*f, model.R0, model.Rr, model.Rc, model.poles_real, model.poles_complex)
    pass
...
def main2():
    ...
    s_params = np.reshape(dict_to_matrix(mzi(wl=1e6*speed_of_light/frequency))[:, 0, 1], (1000, 1, 1))
    ...
    y = ComputeModelResponse((2*np.pi*frequency)/1e13, model.R0, model.Rr, model.Rc, model.poles_real, model.poles_complex)
