...
def state_space_z(poles, residues, feedthrough):
    ...
    A = jnp.zeros((model_order * num_ports, model_order * num_ports), dtype=complex)
...
def main():
    ...
    tic = time()
    poles, residues, feedthrough, error = vector_fitting_z_optimize_order(
    ...
    H = pole_residue_response(
        f, f_center, sampling_frequency, poles, residues, feedthrough
    )
