...
def main():
    ...
    plt.plot(frequency, jnp.angle(siepic.y_branch(1e6*speed_of_light/frequency)[('port_1', 'port_2')]))
    plt.show()
    ...
    poles1, residues1, feedthrough1, error = vector_fitting(50, s_params, 1e-12*(frequency), max_iterations=15, alpha=0.01)
...
def main2():
    ...
    plt.scatter(poles.real, poles.imag, marker='o', facecolors='none', edgecolors='C0', label="Optimal Poles",s=150.0)
    plt.scatter(poles1.real, poles1.imag, marker='x', color='red', label="VF Poles", s=150.0)
    plt.xlabel("REAL")
    plt.ylabel("IMAG")
    plt.legend()
    plt.show()
