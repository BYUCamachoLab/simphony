...
def _fit_to_poles(transfer_function, frequency, poles):
    ...
    transfer_pairs = transfer_function.reshape(
        transfer_function.shape[0], -1
    )  # shape: (num_freq, num_ports*num_ports)
...
def _enforce_conjugacy(poles):
    ...
    p_real = p_sorted[
        ~pos_mask & ~(~pos_mask & (jnp.imag(p_sorted) < -eps))
    ]  # approximately real left over

    # rebuild: interleave for stability (pos, conj), then reattach reals
    rebuilt = jnp.concatenate(
        [
            jnp.ravel(jnp.column_stack([p_pos, p_neg]))
            if p_pos.size
            else jnp.array([], poles.dtype),
            p_real,
        ]
    )
...
def vector_fitting(
    ...
    def relocate_poles(state):
        ...
        current_poles, _ = jnp.linalg.eig(
            A - jnp.ones((model_order, 1)) @ weight_coeffs[:, None].T
        )
        mask = current_poles.real > 0
        current_poles = jnp.where(
            mask, -current_poles.real + 1j * current_poles.imag, current_poles
        )
    ...
    final_poles, *_ = python_based_while_loop(
    ...
    error = _mean_squared_error(
        transfer_function, frequency, final_poles, residues, feedthrough
    )
...
def main():
    ...
    plt.plot(
        frequency,
        jnp.angle(
            siepic.y_branch(1e6 * speed_of_light / frequency)[("port_1", "port_2")]
        ),
    )
    plt.show()

    s_params = jnp.reshape(
    ...
    poles1, residues1, feedthrough1, error = vector_fitting(
        50, s_params, 1e-12 * (frequency), max_iterations=15, alpha=0.01
    )
...
def main2():
    ...
    plt.scatter(
        poles1.real, poles1.imag, marker="x", color="red", label="VF Poles", s=150.0
    )
    plt.xlabel("REAL")
    plt.ylabel("IMAG")
    plt.legend()
