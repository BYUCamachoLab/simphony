...
def run_simulation():
    ...
    inputs = {
        f"o{i}": (
            smooth_rectangular_pulse(t, 0.5e-11, 1.5e-11)
            if i == 0
...
def run_simulation2():
    ...
    inputs = {
        f"o{i}": (
            smooth_rectangular_pulse(newt, 0.5e-11, 1.5e-11)
            if i == 0
            else jnp.zeros_like(t)
        )
        for i in range(num_outputs)
...
def run_simulation3():
    ...
    inputs = {
        f"o{i}": (
            smooth_rectangular_pulse(t, 0.0e-11, 4.0e-11)
            if i == 0
            else jnp.zeros_like(t)
        )
        for i in range(num_outputs)
...
def run_simulation4():
    ...
    inputs = {
        f"o{i}": (
            smooth_rectangular_pulse(t, 0.0e-12, 1.5e-12)
            if i == 0
            else jnp.zeros_like(t)
        )
        for i in range(num_outputs)
...
def test_compare_simulation_results_active_MZI():
    ...
    for key in new_results.keys():
        ...
        assert jnp.allclose(
            new_results[key], golden_results[key], rtol=1e-4, atol=1e-6
