...
class State_Space_Model:
    ...
    def qstep(self, input_states, s=None, t=None, x0_means=None, x0_cov=None, xz0=None):
        # Condition needed to ensure output remains compatible
        system = self.discrete
        # is_ss_input = isinstance(system, StateSpace)
        ...
        if x0_cov is None:
            # xout_cov = np.zeros((A.shape[0], cov.shape[0]))
            xout_cov = np.zeros_like(A)
            # X = np.diag(np.array([1/4]*(A.shape[1]))) * 0.0

        else:
            xout_cov = np.asarray(x0_cov)

        if xz0 is None:
            # xz0 = np.zeros((A.shape[0], cov.shape[0]))
            xz0 = np.zeros_like(A)
            # X = np.diag(np.array([1/4]*(A.shape[1]))) * 0.0

        else:
            xz0 = np.asarray(x0_cov)
        ...
        return yout_means,  yout_cov, xout_means, xout_cov, xz0
