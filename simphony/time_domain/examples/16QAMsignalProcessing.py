...
options = {
    "wl": wvl,
    "wg": {"length": 15.0, "loss": 100},
    "wg2": {"length": 60.0, "loss": 100},
    "wg3": {"length": 50.0, "loss": 100},
}
...
def generate_16qam_piecewise_linear_signal(T, dt, num_symbols, hold_time):
    ...
    if ramp_time < 0:
        raise ValueError(
            "Hold time is too large; must be less than samples_per_symbol."
        )
...
t, I_waveform, Q_waveform, signal_complex = generate_16qam_piecewise_linear_signal(
    T, dt, num_symbols, hold_time
)
...
inputs = {"o0": signal_complex, "o1": jnp.zeros_like(t)}
...
plt.figure(figsize=(6, 6))
plt.plot(
    Q_output, I_output, color="blue", linewidth=1, alpha=0.7, label="Transition Path"
)
# plt.scatter(symbols_I, symbols_Q, color='red', s=50, zorder=5, label='Symbols')
plt.xlabel("In-Phase (I)")
plt.ylabel("Quadrature (Q)")
