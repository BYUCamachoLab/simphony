    ...
    **User-Facing**: Typical usage involves:
            ...
            if self.passive_subnetlists:
                ...
                for i, circuit in sub_circuit_list.items():
                    ...
                    iir_model = IIRModelBaseband(
                        wvl, center_wvl, s_matrix, model_order, options=bvf_options
                    )
    ...
    def init_state(self, **kwargs):
        ...
        if not self._prepared_maps:
            ...
            self._scan_jit = jit(self._scan_loop, static_argnums=(3, 4, 5, 6))
    ...
    def _sample_mode_run(
        self, t: ArrayLike, input_signals: dict
    ) -> TimeResult:  # noqa: D401,E501
        ...
        if not self._prepared_maps:
            ...
            self._scan_jit = jit(self._scan_loop, static_argnums=(4, 5, 6, 7))
            # self._scan_jit = self._scan_loop
            self._prepared_maps = True

        # 2) Pack top‑level inputs into a single JAX array (T × nTop)
        top_inputs = jnp.stack(
            [jnp.asarray(self.inputs[p]) for p in self._port_order],
            axis=1,
        ...
        init_inst = jnp.zeros((n_inst, max_ports, 1), dtype=jnp.complex128)
        init_outs = jnp.zeros(
            (n_steps, len(self._output_specs), 1), dtype=jnp.complex128
        )
        init_states = self.build_initial_states()
        # 4) Execute the fused lax.scan
        final_outs = self._scan_jit(
            top_inputs,
            init_inst,
            init_outs,
            init_states,
    ...
    def _prepare_static_maps(self):
        ...
        self._active_mask = tuple(
            n in (self.time_system_components or set()) for n in self._names
        )
        self._max_ports = (
            max(len(s.ports) for s in self._systems) if self._systems else 0
        )
                    ...
                    if key == key_check:
                        ...
                        row.append(
                            (
                                0,
                                self._inst_idx[si],
                                self._systems[self._inst_idx[si]].ports.index(sp),
                            )
                        )
                        break
                    if value == key_check:
        ...
        for tp in self._port_order:
            ...
            outs.append(
                (
                    self._inst_idx[inst],
                    self._systems[self._inst_idx[inst]].ports.index(prt),
                )
            )
    ...
    def build_initial_states(self):
        ...
        for sys in self._systems:
            if hasattr(sys, "init_state"):
                states_list.append(
                    sys.init_state(dt=self.dt, carrier_freq=self.carrier_freq)
                )
            else:
                states_list.append(())
    ...
    def _scan_loop(
        ...
        def step(carry, t_idx):
            ...
            for inst_i, sys in enumerate(systems):
                ...
                arr = jnp.pad(
                    arr, ((0, max_ports - arr.shape[0]), (0, 0))
                )  # (max_ports, 1)
            ...
            new_rows_list = (
                []
            )  # for collecting each instance’s padded output (max_ports,1)
            ...
            for inst_i, sys in enumerate(systems):
                ...
                new_state_i, local_outputs_tuple = sys.step(
                    prev_state_i,
                    input_tuple,
                    dt=self.dt,
                ...
                arr = jnp.stack(
                    [jnp.atleast_1d(o) for o in local_outputs_tuple], axis=0
                )  # (#local_ports,1)
                arr = jnp.pad(
                    arr, ((0, max_ports - arr.shape[0]), (0, 0))
                )  # (max_ports,1)
        ...
        (final_inst_outs, final_outs, final_states), _ = lax.scan(
            step, (inst0, outs0, states0), jnp.arange(n_steps)
        )
    ...
    def interpolate_inputs(self) -> tuple:
        ...
        if self.dt is None:
            raise ValueError(
                "Time step (dt) not set. Did you forget to call build_model?"
            )
    ...
    def step(self, xprev, *inputs_tuple, **kwargs):
        ...
        def _one_step_body(carry, top_in):
            ...
            for inst_i, sys in enumerate(self._systems):
                ...
                new_state_i, local_outputs = sys.step(
                    prev_state_i,
                    input_tuple,
                    dt=self.dt,
                    carrier_freq=self.carrier_freq,
                ...
                out_arr = jnp.stack(
                    [jnp.atleast_1d(o) for o in local_outputs], axis=0
                )  # (n_local,1)
    ...
    def prepare_time_domain_netlist(self, port_map_list: dict) -> None:
        ...
        for conn_tuple in self.removed_connections:
            ...
            if left_comp in self.time_system_components:
                if left_comp not in self.td_netlist["models"]:
                    ...
                    self.td_netlist["models"][left_comp] = self.models.get(
            ...
            if right_comp in self.time_system_components:
                if right_comp not in self.td_netlist["models"]:
                    ...
                    self.td_netlist["models"][right_comp] = self.models.get(
                        active_model_instance
                    )
        ...
        for idx, ports_dict in port_map_list.items():
            for local_port_label, global_port_str in ports_dict.items():
                ...
                if any(global_port_str in pair for pair in self.removed_connections):
                    for left_str, right_str in self.removed_connections:
                        if left_str == global_port_str:
                            self.add_to_time_domain_netlist(
                                connection=(subckt_key, right_str)
                            )
                        elif right_str == global_port_str:
                            self.add_to_time_domain_netlist(
                                connection=(subckt_key, left_str)
                            )
                else:
        ...
        for k in self.time_system_components:
            ...
            if k not in self.td_netlist["models"]:
                ...
                if active_model_instance:
                    self.td_netlist["models"][k] = self.models.get(
                        active_model_instance
                    )
    ...
    def create_passive_sub_netlists(
        ...
        filtered_ports, removed_ports = self.remove_ports_to_active(
            self.netlist["ports"], active_components
        )
    ...
    def build_sub_netlist(
        ...
        for k_string, v_string in removed_connections:
            ...
            if k_comp in scc_instances or v_comp in scc_instances:
                if (
                    any(
                        k_comp.strip() == value.split(",")[0].strip()
                        for value in removed_ports.values()
                    )
                    and k_comp not in active_components
                ):
                    temp_index = 0
                ...
                if v_comp in active_components:
                    temp_index = 0
                    if any(
                        v_comp.strip() == value.split(",")[0].strip()
                        for value in removed_ports.values()
                    ):
                        for i, j_string in removed_ports.items():
                            j_comp = j_string.split(",")[0]
