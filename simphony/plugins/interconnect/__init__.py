...
def export(model, filename, wl=None, freq=None, clear=True):
    ...
    frequency/wavelength range to a '.txt' file. Any file extension can be
    written onto the filename.

    Must include either the wavelength or frequency
    argument. If both are included, defaults to frequency
    argument.

    Parameters
    -----------
    model : Model
      Any class inheriting from simphony.Model
    filename : str
      The filename (may include path to directory) to save the
      model to. Make sure to include a suffix.
    wl : ndarray, optional
      Wavelengths you want to save sparameters for (in
      meters).
    ...
    Examples
    --------
    We can write the s-parameters for a ``Waveguide``
    instantiated with a length of 100 nanometers to a file
    named ``waveguide.sparams``.

    ```
    import numpy as np
    from simphony.libraries.siepic import Waveguide
    waveguide = Waveguide(100e-9)
    export(waveguide, 'waveguide.sparams', wl=np.linspace(1520e-9, 1580e-9, 51))
    ```
    """
