...
class SpiceVisitor(NodeVisitor):
            ...
            for option in optionset:
                if 'order' in option:
                    ...
                    try:
                        ...
                        ona['params'][option['name']] = _dlist_insert(l, option['order'] - 1, option['value'])
                    except KeyError:
                        ona['params'][option['name']] = _dlist_insert([], option['order'] - 1, option['value'])
                else:
    ...
    def visit_subckt(self, node, visited_children):
        ...
        if header['name'] != footer['name']:
            raise ValueError('Invalid netlist (name in header and footer does not match).')
        name = header['name']
        ports = header['externals']
        params = {p['name']: p['value'] for p in params}
        return Directives.SUBCKT, {'name': name, 'ports': ports, 'components': components, 'params': params}
    ...
    def visit_circuit(self, node, visited_children):
        ...
        return SpiceObjects.CIRCUIT, {'name': cname, 'ports': ports, 'subcircuits': subcircuits, 'params': params}
