...
class SteadyStateSimulationParameters(SimulationParameters):
    ...
    `SimulationParameters` fields. The class exists so components and
    PCells can distinguish steady-state instantiation from S-parameter,
    Block mode, and Sample mode instantiation.
    """
...
class SteadyStateSimulation(Simulation):
    ...
    Steady-state simulations are used directly by users and internally
    by S-parameter simulations to resolve bias/control values before
    evaluating optical scattering responses.
    """
    ...
    def run(
        ...
        """Instantiate the circuit and compute steady-state component
        outputs."""

        instantiated_circuit = self.circuit.instantiate(
            self.settings, self.simulation_parameters
        )
        simulation_result = SteadyStateSimulationResult(instantiated_circuit)

        self.steady_state_order = self._determine_steady_state_order_nx_method(
        ...
        for instance_name in self.steady_state_order:
            ...
            component = instantiated_circuit.instantiated_flat_netlist["instances"][
                instance_name
            ]["model"]
    ...
    def _determine_steady_state_order_nx_method(self, instantiated_circuit):
        ...
        Steady-state components are evaluated once, so the instantiated
        graph must be acyclic. Cycles imply a feedback equation that
        this driver does not currently solve.
        """
        try:
            return list(nx.topological_sort(instantiated_circuit.graph))
        except nx.NetworkXUnfeasible:
            raise ValueError(
                "Failed to determine steady state order – circular dependencies detected"
            )
