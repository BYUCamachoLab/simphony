...
class TestSiepicHash:
    ...
    def test_hash(self):
        ...
        dc1 = siepic.DirectionalCoupler(gap=2.2e-7, Lc=0)
        dc2 = siepic.DirectionalCoupler()
        assert (dc1.__hash__() != dc2.__hash__())
...
class TestSipannHash:
    ...
    def test_hash(self):
        ...
        hr1 = sipann.HalfRing(gap=1.5e-7, radius=10e-6, width=5.2e-7, thickness=2.1e-7)
        hr2 = sipann.HalfRing(gap=2e-7, radius=1e-5, width=5e-7, thickness=2.2e-7)
        assert (hr1.__hash__() != hr2.__hash__())
        ...
        hra1 = sipann.HalfRacetrack(gap=1.5e-7, radius=10e-6, width=5.2e-7, thickness=2.1e-7, length=1e-7)
