...
class TestCircuitSiEPICFormatter:
    def test_parse(self, freqs, mzi4: Circuit):
        spi = os.path.join(
        ...
        with Simulation() as sim1:
            laser = Laser(power=1e-3)
            laser.freqs = freqs
            laser.connect(mzi4._get_components()[1])
            detector = Detector()
            detector.connect(mzi4._get_components()[-2])

            samples1 = sim1.sample()

        with Simulation() as sim2:
            laser2 = Laser(power=1e-3)
            laser2.freqs = freqs
            laser2.connect(mzi42._get_components()[1])
            detector2 = Detector()
            detector2.connect(mzi42._get_components()[-3])

            samples2 = sim2.sample()
