...
class DirectedOpticalModulator(BlockModeComponent):
    ...
    def block_mode_response(self, input_signals, simulation_parameters):
        ...
        for m, mode in enumerate(simulation_parameters.mode_identifiers):
            ...
            fraction_of_power_remaining = jnp.repeat(
                fraction_of_power_remaining[:, None], L, axis=1
            )
...
class OpticalModulator(
    ...
    def s_parameters(
        ...
        return {
            ("o0", "o1"): jnp.sqrt(fraction_of_power_remaining)
            * jnp.exp(1j * phase_shift),
            ("o1", "o0"): jnp.sqrt(fraction_of_power_remaining)
            * jnp.exp(1j * phase_shift),
            ("o0", "o0"): 0,
            ("o1", "o1"): 0,
        }
    ...
    def sample_mode_step(
        ...
        phase_op = jnp.polyval(self.phase_coefficients, voltage)
        absorption_dB = jnp.polyval(self.absorption_coefficients, voltage)
        transfer = jnp.sqrt(10 ** (-absorption_dB * self.length / 10)) * jnp.exp(
            1j * phase_op
        )
        ...
        outputs = {
            "o1": SampleModeOpticalSignal(
                amplitude=transfer * o0_in, wavelength=baseband_wls
            ),
            "o0": SampleModeOpticalSignal(
                amplitude=transfer * o1_in, wavelength=baseband_wls
            ),
        }
    ...
    def steady_state(
        ...
        for i, optical_wl in enumerate(optical_wls):
            ...
            delta_n = self.operating_wl / (2 * jnp.pi * self.length) * phase_op
            phase_shift = (
                2 * jnp.pi / optical_wl * (self.effective_index + delta_n) * self.length
            )
