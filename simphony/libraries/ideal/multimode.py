...
class ModeConverter(
    ...
    """Takes in a signal on a single mode (any mode), and outputs a signal on a
    single, specified mode.

    Will ignore all modes except for the first in the signal passed to
    it
    """

    ports = [Port(name="in", type="optical", directionality="input")] + [
        Port(name="out", type="optical", directionality="output")
    ]
    ...
    def block_mode_response(self, inputs, simulation_parameters):
        ...
        output_mode_index = simulation_parameters.mode_identifiers.index(
            self.output_mode
        )
        ...
        output_amplitude = output_amplitude.at[:, :, output_mode_index].set(
            input_amplitude[:, :, input_mode_index]
        )

        outputs = {
            "out": BlockModeOpticalSignal(
                amplitude=output_amplitude, wavelength=wavelength
            ),
        }
...
def mode_multiplexer(
    ...
    class ModeMultiplexer(SampleModeComponent, BlockModeComponent):
        ports = [
            Port(name=port_name, type="optical", directionality="input")
            for port_name in input_port_names
        ] + [Port(name=output_port_name, type="optical", directionality="output")]
        ...
        def block_mode_response(self, input_signals, simulation_parameters):
            ...
            outputs[output_port_name] = BlockModeOpticalSignal(
                amplitude=jnp.zeros((N, L, M), dtype=complex), wavelength=wavelengths
            )

            for mode_no, in_port_name in enumerate(input_port_names):
                input_amplitude = input_signals[in_port_name].amplitude[:, :, mode_no]
                output_amplitude = (
                    outputs[output_port_name].amplitude[:, :, mode_no] + input_amplitude
                )
                shaped_output_amplitude = (
                    outputs[output_port_name]
                    .amplitude.at[:, :, mode_no]
                    .set(output_amplitude)
                )
                outputs[output_port_name] = BlockModeOpticalSignal(
...
def mode_demultiplexer(
    ...
    class ModeDemultiplexer(SampleModeComponent, BlockModeComponent):
        ports = [Port(name=input_port_name, type="optical", directionality="input")] + [
        ...
        def block_mode_response(self, inputs, simulation_parameters):
            ...
            for i, p in enumerate(self.output_port_names):
                output_amplitude = jnp.zeros(
                    (
                        input_amplitude.shape[0],
                        input_amplitude.shape[1],
                        len(simulation_parameters.mode_identifiers),
                    ),
                    dtype=complex,
                )
                output_amplitude = output_amplitude.at[:, :, i].set(
                    input_amplitude[:, :, i]
                )
