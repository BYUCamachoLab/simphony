...
class OpticalDiscreteFilter(
    ...
    def __init__(
        ...
        *,
        ...
        raise NotImplemented(
            "Need to make sure this component matches the convention expected by the new sample mode and block mode simulators"
        )
    ...
    def _sample_mode_initial_state_iir(self, simulation_parameters):
        ...
        M, L = (
            simulation_parameters.num_optical_modes,
            simulation_parameters.optical_baseband_wavelengths.shape[0],
        )
    ...
    def _sample_mode_step_iir(
        ...
        for mode_idx in range(simulation_parameters.num_optical_modes):
            ...
            for wl_idx, wl in enumerate(input_wavelength):
                ...
                output_amplitude = output_amplitude.at[wl_idx, mode_idx].set(
                    y_single_mode
                )
        ...
        outputs = {
            "in": SampleModeOpticalSignal(
                amplitude=jnp.zeros_like(output_amplitude), wavelength=output_wavelength
            ),
            "out": SampleModeOpticalSignal(
                amplitude=output_amplitude, wavelength=output_wavelength
            ),
        }
    ...
    def block_mode_response(self, inputs: dict, simulation_parameters):
        ...
        for mode_idx in range(simulation_parameters.num_optical_modes):
            ...
            for wl_idx, wl in enumerate(input_wavelength):
                ...
                delta_omega = 2 * jnp.pi * (f - f_center) / f_center
                x_single_mode = (
                    jnp.exp(-delta_omega * n * 1j)
                    * input_amplitude[:, wl_idx, mode_idx]
                )
                y_single_mode = jnp.exp(delta_omega * n * 1j) * lfilter(
                    b_single_mode, a_single_mode, x_single_mode
                )
                output_amplitude = output_amplitude.at[:, wl_idx, mode_idx].set(
                    y_single_mode
                )
...
def bidirectional_discrete_state_space(num_inputs, num_outputs) -> type[Component]:
    """A is nxn, where n is the number of model poles B is nxm, where m is the
    number of ports C is mxn, and D is mxm."""

    if not num_inputs == num_outputs:
        raise ValueError(
            "number of inputs and outputs must match for bidirectional state space models"
        )
...
def discrete_state_space(
    ...
    class DiscreteStateSpace(
        ...
        """A is nxn, where n is the number of poles B is nxm, where m is the
        number of inputs C is pxn, where p is the number of outputs and D is
        pxm.

        Port names are assigned dynamically starting at 'port0_in' /
        'port0_out' and ending in 'p{m-1}_in' / 'p{p-1}_out'

        All input signals should be located in mode 0 (usually called
        the TE MODE), otherwise they will be ignored
        """

        ports = [
            Port(name=port_name, type="optical", directionality="input")
            for port_name in input_port_names
        ] + [
            Port(name=port_name, type="optical", directionality="output")
        ...
        def sample_mode_initial_state(self, simulation_parameters):
            ...
            x = jnp.zeros(
                (len(simulation_parameters.optical_baseband_wavelengths), A.shape[0]),
                dtype=complex,
            )
            return x
        ...
        def sample_mode_step(
            ...
            u = jnp.zeros(
                (
                    len(simulation_parameters.optical_baseband_wavelengths),
                    len(input_port_names),
                ),
                dtype=complex,
            )
            ...
            y = jnp.zeros(
                (
                    len(simulation_parameters.optical_baseband_wavelengths),
                    len(output_port_names),
                ),
                dtype=complex,
            )

            for i, f in enumerate(
                SPEED_OF_LIGHT / simulation_parameters.optical_baseband_wavelengths
            ):
            ...
            for i, port in enumerate(output_port_names):
                ...
                outputs[port] = SampleModeOpticalSignal(
                    amplitude=A_t.reshape(
                        (len(simulation_parameters.optical_baseband_wavelengths), 1)
                    ),
        ...
        def block_mode_response(self, input_signals, simulation_parameters):
            ...
            common_mode_index = simulation_parameters.mode_identifiers.index(
                common_mode
            )
