...
def phaseMatch_analysis(contraDC, simulation_setup, plot=False):
    [
        ...
        lambda_self1,
        lambda_self2,
    ] = lumerical_tools.run_mode(contraDC, simulation_setup)
    ...
    tol = 1e-3
    phaseMatchindex = np.where(abs(phaseMatch - neff_avg) < tol)
    phaseMatchindex = phaseMatchindex[0]
    ...
    if plot == True:
        ...
        plt.plot(
            ...
            neff_avg,
            linewidth=4,
            label="Average waveguides effective index",
        )
        plt.axvline(
            x=lambda_phaseMatch * 1e9,
            linestyle=":",
            label="Contra-directional coupling wavelength",
        )
        plt.legend()
...
def kappa_analysis(contraDC, simulation_setup, waveguides, sim_type="FDTD", close=True):
    ...
    if sim_type == "FDTD":
        [
            ...
            delta_lambda_self2,
            lambda_contra,
        ] = lumerical_tools.run_FDTD(contraDC, simulation_setup, close)
        ...
        [
            ...
            delta_lambda_self2,
            lambda_contra,
        ] = lumerical_tools.run_EME(contraDC, simulation_setup, close)
