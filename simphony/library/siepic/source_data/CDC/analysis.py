...
def phase(contraDC, simulation, plot=False):
    ...
    if plot == True:
        ...
        plt.figure()
        plt.plot(
            contraDC.wavelength[:] * 1e9, thruPhase[0, :], label="Through Port Phase"
        )
...
def group_delay(contraDC, simulation, plot=True):
    ...
    if plot == False:
        ...
        plt.figure()
        plt.plot(
            contraDC.wavelength[:-1] * 1e9,
            1e12 * thruGroupDelay[0, :],
            label="Through Port Group Delay",
        )
        plt.plot(
            contraDC.wavelength[:-1] * 1e9,
            1e12 * dropGroupDelay[0, :],
            label="Drop Port Group Delay",
        )
        plt.legend()
...
def gen_sparams(contraDC, simulation, sfile, run_INTC=True):
    ...
    T11 = T[0][0][:span]
    T11 = np.matrix.transpose(T11)
    T12 = T[0][1][:span]
    T12 = np.matrix.transpose(T12)
    T13 = T[0][2][:span]
    T13 = np.matrix.transpose(T13)
    T14 = T[0][3][:span]
    T14 = np.matrix.transpose(T14)

    T21 = T[1][0][:span]
    T21 = np.matrix.transpose(T21)
    T22 = T[1][1][:span]
    T22 = np.matrix.transpose(T22)
    T23 = T[1][2][:span]
    T23 = np.matrix.transpose(T23)
    T24 = T[1][3][:span]
    T24 = np.matrix.transpose(T24)

    T31 = T[2][0][:span]
    T31 = np.matrix.transpose(T31)
    T32 = T[2][1][:span]
    T32 = np.matrix.transpose(T32)
    T33 = T[2][2][:span]
    T33 = np.matrix.transpose(T33)
    T34 = T[2][3][:span]
    T34 = np.matrix.transpose(T34)

    T41 = T[3][0][:span]
    T41 = np.matrix.transpose(T41)
    T42 = T[3][1][:span]
    T42 = np.matrix.transpose(T42)
    T43 = T[3][2][:span]
    T43 = np.matrix.transpose(T43)
    T44 = T[3][3][:span]
    T44 = np.matrix.transpose(T44)
