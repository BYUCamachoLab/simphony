...
class contra_DC:
    ...
    def set_params(self, PCells_params, ID):
        ...
        for PCell_params in PCells_params:
            if (
                PCell_params["Name"] == "contra_directional_coupler"
                and PCell_params["ID"] == ID
            ):
                            ...
                            [
                                ...
                                OrderedDict(
                                    [
                                        ("@name", "sinusoidal"),
                                        ("@type", "double"),
                                        ("#text", str(device.sinusoidal)),
                                    ]
                                ),
                                OrderedDict(
                                    [
                                        ("@name", "apodization_index"),
                                        ("@type", "double"),
                                        ("#text", str(device.apodization)),
                                    ]
                                ),
                                OrderedDict(
                                    [
                                ...
                                OrderedDict(
                                    [
                                        ("@name", "lambda_start"),
                                        ("@type", "double"),
                                        ("#text", str(simulation.lambda_start * 1e9)),
                                    ]
                                ),
                                OrderedDict(
                                    [
                                        ("@name", "lambda_end"),
                                        ("@type", "double"),
                                        ("#text", str(simulation.lambda_end * 1e9)),
                                    ]
                                ),
                                OrderedDict(
                                    [
                                        ("@name", "lambda_points"),
                                        ("@type", "double"),
                                        ("#text", str(simulation.resolution)),
                                    ]
                                ),
                            ],
                        )
...
def sfilename(device, simulation):
    return (
        "w1=%d,w2=%d,dW1=%d,dW2=%d,gap=%d,p=%d,N=%d,s=%d,a=%.2f,l1=%d,l2=%d,ln=%d.dat"
        % (
            #    return 'w1=%d,w2=%d,dW1=%d,dW2=%d,gap=%d,p=%d,N=%d,s=%d,a=%.2f,l1=%d,l2=%d,ln=%d,sim=%s.dat' % (
            round(device.w1 * 1e9, 10),
            round(device.w2 * 1e9, 10),
            round(device.dW1 * 1e9, 10),
            round(device.dW2 * 1e9, 10),
            round(device.gap * 1e9, 10),
            round(device.period * 1e9, 10),
            device.N,
            device.sinusoidal,
            device.apodization,
...
for ID in IDs:
    ...
    if os.path.exists(sfile):
        ...
        #%% main program
        [waveguides, simulation] = dispersion_analysis.phaseMatch_analysis(
            device, simulation
        )
        device = dispersion_analysis.kappa_analysis(
            device, simulation, waveguides, sim_type="EME", close=False
        )
