...
def contraDC_model(contraDC, simulation_setup, waveguides, plot=False, progress=False):
    ...
    ApoFunc = np.exp(
        -np.linspace(0, 1, num=1000) ** 2
    )  # Function used for apodization (window function)
    ...
    # Waveguides models
    Lambda = np.linspace(
        simulation_setup.lambda_start,
        simulation_setup.lambda_end,
        num=simulation_setup.resolution,
    )
    ...
    if a != 0:
        ...
        profile = (ApoFunc - min(ApoFunc)) / (
            max(ApoFunc) - (min(ApoFunc))
        )  # normalizes the profile
    ...
    randomChirpFrac = np.random.rand(1, N_seg) * rch / 100
    randomChirpFrac = randomChirpFrac[0, :]
    ...
    if plot == True:
        ...
        plt.plot(
            zaxis * 1e6,
            couplingChirpFrac + lengthChirpFrac + randomChirpFrac,
            label="Total chirp",
        )
    ...
    if progress:
        ...
        printProgressBar(
            0, progressbar_width, prefix="Progress:", suffix="Complete", length=50
        )
    ...
    for ii in range(lengthLambda):
        ...
        for n in range(N_seg):
            ...
            beta_del_1 = (
                beta_left * chirpWL.item(n) - math.pi / period - j * alpha_e / 2
            )
            beta_del_2 = (
                beta_right * chirpWL.item(n) - math.pi / period - j * alpha_e / 2
            )

            # S1 = Matrix of propagation in each guide & direction
            S_1 = [
                [j * beta_del_1.item(ii), 0, 0, 0],
                [0, j * beta_del_2.item(ii), 0, 0],
                [0, 0, -j * beta_del_1.item(ii), 0],
                [0, 0, 0, -j * beta_del_2.item(ii)],
            ]

            # S2 = transfert matrix
            S_2 = [
                [
                    -j * beta_del_1.item(ii),
                    0,
                    -j * kappa_11 * np.exp(j * 2 * beta_del_1.item(ii) * L_0),
                    -j
                    * kappa_12
                    * np.exp(j * (beta_del_1.item(ii) + beta_del_2.item(ii)) * L_0),
                ],
                [
                    0,
                    -j * beta_del_2.item(ii),
                    -j
                    * kappa_12
                    * np.exp(j * (beta_del_1.item(ii) + beta_del_2.item(ii)) * L_0),
                    -j * kappa_22 * np.exp(j * 2 * beta_del_2.item(ii) * L_0),
                ],
                [
                    j * np.conj(kappa_11) * np.exp(-j * 2 * beta_del_1.item(ii) * L_0),
                    j
                    * np.conj(kappa_12)
                    * np.exp(-j * (beta_del_1.item(ii) + beta_del_2.item(ii)) * L_0),
                    j * beta_del_1.item(ii),
                ...
                [
                    ...
                    * np.conj(kappa_12)
                    * np.exp(-j * (beta_del_1.item(ii) + beta_del_2.item(ii)) * L_0),
                    j * np.conj(kappa_22) * np.exp(-j * 2 * beta_del_2.item(ii) * L_0),
                    0,
                    j * beta_del_2.item(ii),
                ],
            ]
            ...
            P0 = np.matmul(
                scipy.linalg.expm(np.asarray(S_1) * l_seg),
                scipy.linalg.expm(np.asarray(S_2) * l_seg),
            )
            if n == 0:
                P1 = P0 * P
            else:
