...
def model_vs_samples(H, H_hat):
    ...
    plt.plot(x / (2 * np.pi), y, label = "Samples")
    plt.legend()
    plt.show()

    # y_hat = np.arctan(np.imag(H_hat)/np.real(H_hat)) * 180 / np.pi
    # y = np.arctan(np.imag(H)/np.real(H)) * 180 / np.pi
    # plt.plot(x / (2 * np.pi), y_hat, label = "Model")
...
if __name__ == '__main__':
    ...
    # H_hat = estimate_H(poles, residues, x, order, num_samples, 1, 1)
    ...
    num_samples = 10
    sample_freqs = omega_Trad_per_sec

    # for n in range(38, 50,  2):
    #     poles, residues, fitting_error, num_iterations, status = vector_fitting(H = H, freq_THz = sample_freqs, order=n, max_iterations=500, epsilon_w=.5)
    #     print(f"Order: {n}")
    #     print(f"Status: {status}")
    #     print(f"Iterations: {num_iterations}")
    #     print(f"Fitting error squared: {fitting_error}")
    
    best_order = 40
    # sample_freqs = np.linspace(1.5, 1.6, 100)
    poles, residues, fitting_error, num_iterations, status = vector_fitting(H = H, freq_THz = sample_freqs, order=best_order, max_iterations=500, epsilon_w=4.0, alpha=.01, smooth=False)

    num_points = 100
    wvl = np.linspace(1.5, 1.6, num_points)
    
    freq_THz = (c / wvl) * 1e-6
    omega_Trad_per_sec = 2 * np.pi * freq_THz
    x = omega_Trad_per_sec

    H_hat = estimate_H(poles, residues, x, best_order, num_points, 1, 1)
    model_vs_samples(H[:, 0, 0], H_hat[:, 0, 0])
